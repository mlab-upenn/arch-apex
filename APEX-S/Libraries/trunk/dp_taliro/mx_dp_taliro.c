...
/* Modified by Hengyi Yang, ASU, U.S.A. for dp_taliro                     */
...
{	
	...
	/* Peer reviewed on 2013.06.09 by Dokhanchi, Adel */
	...
	for(jstruct = 0; jstruct < NElems; jstruct++) 
	{
		...
		if(par_on)
		{
			...
			/* GF: Get normalization interval */
			/* GF: Any changes here must be propagated to the predicate reading */
		...
		else
		{
			...
			/* GF: Get normalization interval */
			/* GF: Any changes here must be propagated to the parameter reading */
			tmp = mxGetField(prhs[1], jstruct, "Normalized");
			if (tmp != NULL)
			{
				miscell->predMap[jstruct].Normalized = mxGetScalar(tmp)>0.5;
				/* GF: predMap is used in DynamicProgramming to set tmpsymb : Can we replace it with miscell? */
